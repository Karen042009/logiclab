...
def home():
    ...
    return render_template(
        "index.html", courses=all_courses, instructors=all_instructors
    )
...
def ml_project_detail(project_id):
    ...
    if project.get("files"):
        ...
        if student_folder:
            code_dir = os.path.join(
                app.config["PROJECTS_FOLDER"], student_folder, "project", "code"
            )
            for code_file in [
                "main.py",
                "names.py",
    ...
    return render_template(
        "ml_project_detail.html", project=project, course=course, code_files=code_files
    )
...
def serve_project_file(student_folder, filename):
    ...
    try:
        project_root = os.path.join(
            app.config["PROJECTS_FOLDER"], student_folder, "project"
        )
...
def user_login():
    if request.method == "POST":
        ...
        decoded_key = DATA_MANAGER._decode_api_key(str(api_key), expiration_months=13)
...
def user_dashboard():
    ...
    return render_template(
        "users/profile.html", course_id=course_id, materials=materials_data
    )
...
def admin_dashboard():
    ...
    for course in all_courses:
        ...
        course["rejected_count"] = registrations_collection.count_documents(
            {"course_id": course_id, "status": "rejected"}
        )
    ...
    recent_visitors = list(
        DATA_MANAGER.get_visits().find().sort("timestamp", -1).limit(5)
    )
    return render_template(
...
def admin_edit_course(course_id):
    try:
        ...
        if request.method == "POST":
            ...
            try:
                updates = {
                    ...
                    "monthly_payment": (
                        int(request.form.get("monthly_payment", 0))
                        if request.form.get("monthly_payment")
                        else None
                    ),
                    "total_payment": (
                        int(request.form.get("total_payment", 0))
                        if request.form.get("total_payment")
                        else None
                    ),
                ...
                while f"chapter_{chapter_index}_title" in request.form:
                    ...
                    if chapter_title is None or not chapter_title.strip():
                        chapter_index += 1
                        continue

                    chapter_content = request.form.get(
                        f"chapter_{chapter_index}_content", ""
                    )
                    chapter_data = {
                        "title": chapter_title,
                        "content": [
        ...
        return render_template(
            "admin/edit_course.html", course=course, instructors=instructors
        )
    ...
    if request.method == "POST":
        try:
            ...
            for field in required_fields:
                if not request.form.get(field):
                    flash(
                        f'{" ".join(field.split("_")).capitalize()} is required',
                        "error",
                    )
            ...
            while True:
                ...
                companies.append({"company": name, "type": company_type, "role": role})
                i += 1
            ...
            new_instructor = {
                ...
                "softwareProficiency": [
                    software.strip()
                    for software in request.form.get("softwareProficiency", "").split(
                        ","
                    )
                    if software.strip()
                ],
...
def admin_edit_instructor(instructor_id):
    ...
    if request.method == "POST":
        try:
            ...
            photo_url = (
                photo_url if is_valid_url(photo_url) else DEFAULT_INSTRUCTOR_PHOTO
            )
            # Basic info
            updates = {
                "firstName": request.form.get("firstName"),
            ...
            updates["softwareProficiency"] = [
                s.strip() for s in software.split(",") if s.strip()
            ]

            # Update the instructor
            instructors_collection.update_one({"_id": instructor_id}, {"$set": updates})
            flash("Instructor updated successfully", "success")
            return redirect(
                url_for("admin_edit_instructor", instructor_id=instructor_id)
            )
...
def admin_students():
    ...
    all_courses = list(courses_collection.find())
    return render_template(
        "admin/students.html", students=all_students, courses=all_courses
    )
...
def admin_update_student(student_id):
    ...
    registrations.update_one(
        {"_id": ObjectId(student_id)}, {"$set": {"status": new_status}}
    )
    ...
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        return jsonify(
            {"success": True, "email_sent": email_sent, "new_status": new_status}
        )
    ...
    if request.method == "POST":
        try:
            ...
            for field in required_fields:
                if not request.form.get(field):
                    flash(
                        f'{field.replace("_", " ").capitalize()} is required', "error"
                    )
            ...
            while True:
                ...
                chapters.append({"title": chapter_title, "content": lessons})
                chapter_index += 1
        ...
        return jsonify(
            {
                ...
                "message": (
                    ...
                    if success
                    else "Failed to update materials"
                ),
            }
        )
    ...
    try:
        if request.is_json:
            ...
            lesson_data = {
                ...
                "presentations": [
                    url.strip()
                    for url in request.form.getlist("presentations[]")
                    if url.strip()
                ],
                "code": [
                    url.strip() for url in request.form.getlist("code[]") if url.strip()
                ],
                "other": [
                    url.strip()
                    for url in request.form.getlist("other[]")
                    if url.strip()
                ],
            }
    ...
    if request.method == "POST":
        try:
            ...
            for idx, viz_file in enumerate(viz_files):
                if (
                    viz_file
                    and viz_file.filename
                    and allowed_file(viz_file.filename, ALLOWED_IMAGE_EXTENSIONS)
                ):
                    ...
                    visualizations.append(
                        {
                            "title": (
                                viz_titles[idx] if idx < len(viz_titles) else filename
                            ),
                            "image_path": f"materials/{filename}",
                            "description": (
                                viz_descs[idx] if idx < len(viz_descs) else ""
                            ),
                        }
                    )
            ...
            project_data = {
                ...
                "technologies": [
                    tech.strip()
                    for tech in request.form.get("technologies", "").split(",")
                    if tech.strip()
                ],
...
def admin_edit_ml_project(project_id):
        ...
        try:
            ...
            updated_data = {
                ...
                "files": {
                    "data": [
                        url.strip()
                        for url in request.form.getlist("data_files[]")
                        if url.strip()
                    ],
                    "materials": [
                        url.strip()
                        for url in request.form.getlist("materials_files[]")
                        if url.strip()
                    ],
                    "models": [
                        url.strip()
                        for url in request.form.getlist("models_files[]")
                        if url.strip()
                    ],
    ...
    all_courses = list(courses_collection.find({"is_active": True}))
    return render_template(
        "admin/edit_ml_project.html", project=project, courses=all_courses
    )
...
def get_all_materials():
    try:
        ...
        materials = {
            "machine_learning": DATA_MANAGER.get_materials_by_course_id(
                "machine_learning"
            )[0],
